...
try {
	...
	$valor_detalhado = "SELECT venda.data_venda AS data_da_venda, produtos_venda.cd_produto, compra_produto.nome,
	produtos_venda.valor_item, SUM(produtos_venda.quantidade) AS qtd_vendida, 
	produtos_venda.valor_venda AS venda_mes FROM venda
	...
	ORDER BY data_da_venda, produtos_venda.cd_produto";
	...
	$valor_total = "SELECT YEAR(venda.data_venda) AS ano, SUM(produtos_venda.valor_venda) AS venda_total FROM venda
	INNER JOIN produtos_venda ON (produtos_venda.cd_venda = venda.cd_venda)
	ORDER BY YEAR(venda.data_venda)";
							...
							<div class="col-md-12">
								<div class="card card-plain table-plain-bg">
									<div class="card-header ">
										<center>
											<h4 class="card-title">Fluxo de vendas</h4>
											</br>
							...
							<table class="table">
								<thead>
									<tr>
										...
										<th scope="col" title="Quantidade vendida"> Quantidade vendida </th>
								...
								<tbody>
									...
									foreach ($linhas_detalhes as $exibir_colunas) {
										echo '<tr>';
										echo '<th scope="row" title="' . date('d/m/Y H:i:s', strtotime($exibir_colunas['data_da_venda'])) . '">' .
											date('d/m/Y H:i:s', strtotime($exibir_colunas['data_da_venda'])) . '</th>';
